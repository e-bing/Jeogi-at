...
* **공기질 센서 모니터링 (CO/CO2)**
  * STM32와 UART 프로토콜로 통신하여 CO/CO2 센서 데이터를 1.2초 주기로 수집하고 MQTT로 발행합니다.
...
* **환기 모터 제어**
  ...
  * 자동 모드: CO2/CO 임계값에 따라 속도를 자동 조절 (예: CO2 > 600ppm → 100%, CO2 > 400ppm → 60%)
...
* **MQTT 기반 서버 통신**
  * 센서 데이터(공기질, 온습도, 시스템 상태)를 서버에 발행하고, 모터/오디오/디스플레이 제어 명령을 구독합니다.
