...
void init_mqtt_motor() {
  ...
  try {
    // 실제 함수가 호출되는 시점에 객체를 생성 (이때 g_mqtt_broker 값을 읽음)
...
void send_motor_command(const string& action, int speed) {
  ...
  try {
    json cmd = {{Protocol::FIELD_TYPE, Protocol::MSG_MOTOR_CONTROL},
...
void send_mode_command(const string& mode) {
  ...
  try {
    json cmd = {{Protocol::FIELD_TYPE, Protocol::MSG_MODE_CONTROL},
