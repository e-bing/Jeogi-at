  ...
  if (++db_tick >= 100)
  {
    ...
    try
    {
...

      {
        ...
        if (SSL_write(ssl, s1.c_str(), s1.length()) <= 0)
          break;
        if (SSL_write(ssl, s2.c_str(), s2.length()) <= 0)
          break;
        if (SSL_write(ssl, s3.c_str(), s3.length()) <= 0)
          break;
        if (SSL_write(ssl, s4.c_str(), s4.length()) <= 0)
          break;
                        ...
                        const vector<unsigned char> &img_data)
{
  ...
  if (SSL_write(ssl, &header, sizeof(header)) <= 0)
    return false;
  if (!json_str.empty())
    if (SSL_write(ssl, json_str.c_str(), json_str.size()) <= 0)
      return false;
  if (!img_data.empty())
    if (SSL_write(ssl, img_data.data(), img_data.size()) <= 0)
      return false;
...
void video_streaming_worker(SSL *ssl, bool *client_connected)
{
  while (*client_connected)
  {
    // 1. Hanwha 카메라 데이터 전송 (ID: 1, 15 FPS)
    {
      ...
      if (!jpg_buffer.empty())
      {
        if (!send_camera_packet(ssl, 1, json_payload, jpg_buffer))
          break;
      }
    }
