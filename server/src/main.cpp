...
int main() {
  ...
  //     renderer, SDL_PIXELFORMAT_IYUV, SDL_TEXTUREACCESS_STREAMING, 1920,
  //     1080);
  ...
  while (running && !stop_flag) {
    ...
    //             hwTexture, nullptr, g_hw_frame_buffer.data(), real_w,  // Y
    //             pitch g_hw_frame_buffer.data() + (real_w * real_h), real_w /
    //             2,
    //             // U pitch g_hw_frame_buffer.data() + (real_w * real_h * 5 /
    //             4), real_w / 2  // V pitch
    //         );
    //       }
    ...
    //         SDL_UpdateYUVTexture(piTexture, nullptr, y_plane, w, u_plane, w /
    //         2,
    //                              v_plane, w / 2);
    ...
    //   //     SDL_Rect r = {
    //   //         (int)(obj.x + 640),  // 영상이 오른쪽 절반에 있으므로 640
    //   더하기
    ...
    // 한 줄에 갱신하며 출력 (서버 상태 모니터링용)
    cout << "\r[SYSTEM] Server Running | Pi Nodes: " << g_pi_node_map.size()
         << " | Total People: " << (g_hw_objects.size() + total_pi) << "  "
         << flush;
    //   int pi_count = 0;
    //   int hw_count = 0;
    ...
    //       // (선택 사항) SDL 화면에 모든 노드의 박스를 다 그리고 싶다면
    //       여기서
    //       // DrawRect 실행
